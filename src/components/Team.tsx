      ...
      >
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4">
            ...
            <p className="text-[10px] text-white/70 sm:text-xs">Touchez pour voir le détail</p>
          </div>
        </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <motion.div {...anim} className="text-center">
          ...
          <p className="mx-auto mt-3 max-w-2xl text-sm text-muted-foreground sm:text-base">
            Touchez ou survolez un praticien pour découvrir son parcours.
          </p>
        </motion.div>

        <div className="mt-8 grid grid-cols-2 gap-3 sm:mt-10 sm:gap-4 lg:grid-cols-5">
          {doctors.map((doc, i) => (
            <div key={doc.name} className={i === doctors.length - 1 ? "col-span-2 mx-auto w-1/2 lg:col-span-1 lg:w-full" : ""}>
              <FlipCard doc={doc} index={i} />
            </div>
          ))}
        </div>
      </div>
